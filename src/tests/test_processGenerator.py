...
def test_processGenerator():
    ...
    for i in range(10):
        for j in range(3):
            assert p_list.incoming[i].io_bursts[j] <= 6.4 and p_list.incoming[i].io_bursts[j] >= 1.0 
        for j in range(4):
            assert p_list.incoming[i].cpu_bursts[j] <= 10.0 and p_list.incoming[i].cpu_bursts[j] >= 1.0
        assert p_list.incoming[i].arrival_time <= 2.0